...
## 🎉 最終完了（2026-04-16 14:56 JST）

### 稼働中の機能
- Slack → Next.js API → Google Sheets の自動転記（リアルタイム）
- 毎朝 07:00 JST + 夜 22:00 JST に Vercel Cron で自動バックフィル
- バックフィル末尾で完全重複行を自動削除
- 生データシートに赤ハイライトで重複可視化
- 担当者名の正規化（Slack ID→漢字名）

### エンドポイント（運用中いつでも使用可）
- `/api/slack/health` — 環境変数チェック
- `/api/slack/duplicates` — 重複検出（読み取り専用）
- `/api/slack/backfill` — 手動バックフィル＋自動dedupe
- `/api/slack/cleanup?zeroRows=X&deleteRows=Y` — 行ゼロ化 / 削除
- `/api/slack/fix-names` — 担当者名正規化
- `/api/slack/setup-formatting` — 条件付き書式再設定

### 呼び出し方
```bash
curl -H "Authorization: Bearer pcn-cron-2026-04-16-x9k3m7" \
  https://pcn-dashboard-app.vercel.app/api/slack/<endpoint>
```

### 今後の運用
- **日常**: 何もしなくてOK（自動運用）
- **数字がおかしい時**: `/api/slack/duplicates` で確認 → 必要なら `cleanup` で修正
- **新担当者追加**: `lib/slack/parse.js` の `NAME_CORRECTIONS` に追記 → git push

### 残置物（廃止予定）
- 旧GAS `pcn_slack_to_sheets`: Slack側URL変更で呼ばれなくなった。削除しなくてもOK（参考用に残置）
