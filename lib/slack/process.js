...
import { dedupeRawData } from './dedupe.js';
...
async function threadHasHumanReplyWithBranchData(channel, threadTs) {
  try {
    const data = await slackApi('conversations.replies', {
      channel, ts: threadTs, limit: '50',
    });
    return (data.messages || []).some((m) =>
      m.ts !== threadTs && !m.bot_id && hasBranchData(getTextFromEvent(m))
    );
  } catch (e) {
    console.error('threadHasHumanReplyWithBranchData error', e);
    return false;
  }
}
...
async function handleDefinedSokan(event, text) {
  ...
  // 人間返信優先: bot 定性所感イベントの場合、同スレッドに人間返信(支店データ)が既にあれば無視
  if (event.bot_id === CONFIG.SLACK_BOT_ID) {
    const hasHuman = await threadHasHumanReplyWithBranchData(event.channel, threadTs);
    if (hasHuman) {
      console.log(`skip bot 定性所感 (human reply present): thread=${threadTs}`);
      return;
    }
  }
...
export async function backfillRecentThreads(limit = 50) {
  ...
  for (const msg of messages) {
    ...
    // 人間返信優先: スレッド内に人間返信(支店データ)があればそれだけを処理、bot定性所感は無視
    const humanRepliesWithData = (threadData.messages || []).filter(
      (r) => r.ts !== msg.ts && !r.bot_id && hasBranchData(getTextFromEvent(r))
    );
    const sourceReplies = humanRepliesWithData.length > 0
      ? humanRepliesWithData
      : (threadData.messages || []).filter(
          (r) => r.ts !== msg.ts && r.bot_id === CONFIG.SLACK_BOT_ID && getTextFromEvent(r).includes('定性所感')
        );
  ...
  // バックフィル末尾で自動dedupe
  let deduped = { deleted: 0, rows: [] };
  try {
    deduped = await dedupeRawData();
    if (deduped.deleted > 0) {
      console.log(`auto-dedupe: deleted ${deduped.deleted} duplicate rows`, deduped.rows);
    }
  } catch (e) {
    console.error('auto-dedupe failed', e);
  }

  return { processed, skipped, deduped };
}
